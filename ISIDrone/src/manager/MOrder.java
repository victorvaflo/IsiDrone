...
public class MOrder {
	public static int add(User user, Cart cart) {
		...
		try {
			...
			String query = "INSERT INTO `order` (`user_id`, `date`,`isShipped`) VALUES (?, now(), ?)";
	...
	public static List<Order> getAllOrdersByUserId(int userId){
		...
		try {
			...
			String query = "SELECT `order`.id, `order`.date, `isShipped`"
        ...
        try {
            MDB.connect();
            String deletQuerry = " DELETE FROM `order` WHERE id = ?";
